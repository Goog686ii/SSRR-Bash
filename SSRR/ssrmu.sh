...
Download_SSR(){
	...
	env GIT_SSL_NO_VERIFY=true git clone -b akkariiin/master https://github.com/shadowsocksrr/shadowsocksr.git
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/Chennhaoo/Shell_Bash/master/Server/ssrmu_centos -O /etc/init.d/ssrmu; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/Chennhaoo/Shell_Bash/master/Server/ssrmu_debian -O /etc/init.d/ssrmu; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssrmu
	...
	do
		...
		if [[ ! -z ${Clear_transfer_user} ]]; then
			...
			if [[ -z "${match_clear}" ]]; then
				...
				Get_User_info "${Clear_transfer_user_port}"
				View_User_info
			fi
			break
		else
...
if [[ "${action}" == "clearall" ]]; then
	...
	echo -e "  ShadowsocksRR MuJSON后端一键管理脚本 ${Green_font_prefix}[MOD_${sh_ver} 181204]${Font_color_suffix} 
...
————————————
 ${Green_font_prefix}14.${Font_color_suffix} 修改SSH端口 （如有宝塔面板，不建议在此修改）
